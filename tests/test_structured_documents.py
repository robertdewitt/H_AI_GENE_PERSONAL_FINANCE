...
class TestParseDocuments:
    ...
    def test_parse_rental_sample_file(self):
        ...
        assert parsed.net_bank_deposit == 310.0
...
class TestApplyPayrollDocument:
    def test_apply_sample_creates_splits(self, db):
        ...
        non_cash_split = [s for s in txn.splits if s.amount_native == 0.0]
...
class TestApplyRentalDocument:
    def test_apply_sample_pnl_snapshot(self, db):
        ...
        assert snap.total_income == 2400.0
        assert snap.total_expense == 590.0
        assert snap.owner_draw == 1500.0
        assert snap.liability_adjustment == 600.0
        assert snap.net_operating_income == 1810.0
        assert snap.net_cash_flow == 310.0
